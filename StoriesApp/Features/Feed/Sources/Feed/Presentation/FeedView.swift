...
struct FeedView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            topMenu
            ZStack {
                // Next card (bottom of stack) - always at full size and opacity
                if let nextProfile = viewModel.nextProfile {
                    FeedCardView(profile: nextProfile)
                        .id(nextProfile.id)
                }
